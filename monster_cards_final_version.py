...
function and an option. Tidied up the comments and program aesthetics.
...
def main(catalogue, stat_list):
    ...
    while True:
        # Gets a copy of the catalogue before new changes are made
        ...
        choice = easygui.buttonbox("What would you like to do?",
                                   ...
                                   ["Add card", "Search card",
                                    ...
                                    "Instructions", "Exit"])
        if choice == "Add card":
            # This gets the new cards dictionary and 'added' stores if the user
            ...
            # This gets the result of the edit menu (None or updated catalogue)
            new_edited_result = editing_menu(new_card_dict, formatted_new_card,
                                             added, stat_list, catalogue)

            # If the result isn't None it must have been confirmed
            if new_edited_result is not None:
                # Catalogue is updated with new details
                catalogue = new_edited_result
            else:
                # If None was returned (cancelled) the catalogue is replaced
                # with the copy to ensure the edits weren't updated
            ...
            searched_card_dict, search_marker = search_card(sorted_catalogue,
                                                            "Search "
                                                            "catalogue")
            ...
            # This gets the result of the edit menu (None or updated catalogue)
            searched_edited_result = editing_menu(searched_card_dict,
                                                  formatted_searched_card,
                                                  search_marker, stat_list,
                                                  catalogue)

            # If the result isn't None it must have been confirmed
            if searched_edited_result is not None:
                # Catalogue is updated with changed details
                catalogue = searched_edited_result
            else:
                # If None was returned (cancelled) the catalogue is replaced
                # with the copy to ensure the edits weren't updated
                catalogue = copied_catalogue
...
def editing_menu(card_dict, formatted_card_dict, choice_marker, stat_list,
    ...
    # This keeps track of the original card name for if user confirms changes
        ...
        # Returns the edited card dictionary and True marker if they confirm
            ...
            if original_card_name in catalogue:
                ...
                if original_card_name in card_dict:
                    ...
                    # Adds new card and details to the catalogue
                    catalogue.update(card_dict)

            else:
                # If card is new adds its details to the catalogue
                catalogue.update(card_dict)
